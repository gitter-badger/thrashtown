...
var setupSurf = function (user) {
  var uId = user._id;
  var boardId = user.boards[0]._id;
  var surfSpotId = user.surfSpots[0]._id;
  Surf.create(
...
var clean = function() {
  console.log("Wiping out database");
  ...
  var surfs = new Promise((resolve, reject) => {
    Surf.find({}).remove( (err) => {
      if (err) return reject();
      return resolve();
    });
  });
  return Promise.all([users, surfs]);
};
...
var createUsers = function() {
  ...
  return Promise.all([friend, test, admin]);
};

clean().then(createUsers).then( users => {
  console.log("Setting up surfs");
  ...
  setupSurf(test);
  setupSurf(friend);
  test.friends = [friend._id];
  test.save();

  console.log(`User ${test.email} created with quiver and spots. Id: ${test.id}`);
  console.log(`User ${friend.email} created with quiver and spots. Id: ${friend.id}`);
  console.log(`User ${admin.email} created with quiver and spots. Id: ${admin.id}`);
